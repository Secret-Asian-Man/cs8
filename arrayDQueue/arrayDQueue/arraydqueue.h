#ifndef ARRAYDQUEUE_H
#define ARRAYDQUEUE_H
...
enum DQUEUE_ERRORS {DQUEUE_EMPTY, DQUEUE_FULL, DQUEUE_ILLEGAL_SIZE};
...
{
public:
    arrayDQueue(uint s = 100);
    arrayDQueue(const T &data, uint s = 100);
    arrayDQueue(const arrayDQueue<T>& other);
    ~arrayDQueue();

    arrayDQueue<T>& operator=(const arrayDQueue<T>& other);
    ...
    arrayDQueue<T>& operator<<(const T& d);
    arrayDQueue<T>& operator>>(T& d);
    void merge(const arrayDQueue<T> &other);

    template<typename U>
    friend
    ostream& operator<<(ostream &out, const arrayDQueue<U> &q);

    template<typename U>
    friend
    istream& operator>>(istream &in,  arrayDQueue<U> &q);
...
private:
    ...
    void nukem();
    void copy(const arrayDQueue<T> &other);
};

template<typename T>
arrayDQueue<T>::arrayDQueue(uint s)
{
    size=0;
    maxsize=s;
    head= new T[size];
...
template<typename T>
arrayDQueue<T>::arrayDQueue(const T &data, uint s)
{
    if(s < 1)
        throw DQUEUE_ILLEGAL_SIZE;
...
template<typename T>
arrayDQueue<T>::arrayDQueue(const arrayDQueue<T>& other)
{
    size=0;
    maxsize=100;
    head= new T[size];
...
template<typename T>
arrayDQueue<T>& arrayDQueue<T>::operator=(const arrayDQueue<T>& other)
{
    if(this != &other)
    {
        nukem();
...
template<typename T>
bool arrayDQueue<T>::full()
{
    return size >= maxsize;
}

template<typename T>
bool arrayDQueue<T>::empty()
{
    return (size==0);
}

template<typename T>
void arrayDQueue<T>::clear()
{
    nukem();
}

template<typename T>
uint arrayDQueue<T>::getSize()
{
    return size;
}

template<typename T>
void arrayDQueue<T>::resize(uint s)
{
    if(size > s)
        throw DQUEUE_ILLEGAL_SIZE;
...
template<typename T>
void arrayDQueue<T>::pushAtBack(const T& d)
{
    if(full())
        throw DQUEUE_FULL;
...
template<typename T>
void arrayDQueue<T>:: pushAtFront(const T &d)//DEQUEUE FUNCTION
{
    if(full())
        throw DQUEUE_FULL;
...
template<typename T>
T arrayDQueue<T>::popAtBack()//DEQUEUE FUNCTION
{
    if (empty())
        throw DQUEUE_EMPTY;
    T returnMe=T();
...
template<typename T>
arrayDQueue<T>& arrayDQueue<T>::operator<<(const T& d)
{
    pushAtBack(d);
    return *this;
}
...
template<typename T>
arrayDQueue<T>& arrayDQueue<T>::operator>>(T& d)
{
    d=popAtFront();
    return *this;
}
...
template<typename T>
void arrayDQueue<T>::merge(const arrayDQueue<T> &other)
{
...
template<typename T>
void arrayDQueue<T>::copy(const arrayDQueue<T> &other)
{
    nukem();
