...
void perform(bst<char> tree[], char selection, int &touse)
{
    ...
    case 'S' : do
        {
            cout<<"Enter 1 for a PREORDER, 2 for INORDER, 3 for POSTORDER, 4 for REVERSEINORDER";
            cin>>order;
        ...
        case 3: tree[touse].setTraversal(POST_ORDER);
            break;
        case 4: tree[touse].setTraversal(REVERSE_IN_ORDER);
            break;
